...
suite('Ractive.defaults', function (Ractive) {
  ...
  it('picks up shared objects from the start', function () {
    expect(one.get('audio.volume')).toEqual(30);
    expect(two.get('audio.volume')).toEqual(30);
    expect(audio.get('volume')).toEqual(30);
  });

  it('picks up changes from the shared object', function () {
    audio.set('volume', 60);
    expect(one.get('audio.volume')).toEqual(60);
    expect(two.get('audio.volume')).toEqual(60);
    expect(audio.get('volume')).toEqual(60);
  });
  ...
  it('allows you to set the data via the wrapper', function () {
    ...
    expect(one.get('audio.volume')).toEqual(70);
    expect(two.get('audio.volume')).toEqual(70);
    expect(audio.get('volume')).toEqual(70);
  });
});
